...
show_help = (msg) ->
    ...
    default_db = config.default_database
...
#    msg.send "Setting #{alert_id} #{alert_key}"
    ...
    query_config = config.queries[database][query_name]
...
process_alert = (robot, msg, query_name, data, columns) ->
    ...
    if alert
        ...
        if alert_obj.status == ACK
            ack_ts = alert_obj.ack_ts
            cur_ts = (new Date).getTime()
            if cur_ts - ack_ts > _1_HOUR_MS*HOURS_BEFORE_ACK_REPOST
                 new_alert(robot, msg, query_name, data, columns)
#            else
#                 msg.send "Not sending, acked"
            ...
            cur_ts = (new Date).getTime()
            if cur_ts - created_ts > _1_HOUR_MS*HOURS_BEFORE_REPOST
                 new_alert(robot, msg, query_name, data, columns)
            else
...
connect = ->
    ...
    influx_connect_config = config.connection
    for database in Object.keys(config.queries)
...
check_for_alerts = (robot, msg) ->
    connect()
    for database in Object.keys(config.queries)
        for query_name in Object.keys(config.queries[database])
            query_config = config.queries[database][query_name]
